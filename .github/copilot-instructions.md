---
description: "HoneyHub Platform Copilot Instructions"'
applyTo: '**/*.cs,**/*.csproj,**/Program.cs,**/*.json,**/*.md,**/*.yml,**/*.yaml,**/*.config,**/*.cshtml,**/*.razor,**/*.js,**/*.ts,**/*.css,**/*.scss,**/*.html'
---
...
## Review mindset
- **Be a teammate.** Point out issues, then suggest one concrete fix or better pattern.
- **No tutorials.** Skip 101 explanations unless explicitly asked.
- **Compact diffs.** Prefer patch suggestions (` ```diff `) over prose when showing changes.
- **Positive first.** Call out at least one thing done well before listing improvements.

## Naming & structure
- All entity classes end with **`Entity`**; DTOs use the base name (e.g., `User`).
- Table names are **singular** to match entity names.
- No kebab‑case in routes; use **PascalCase** segments (`/api/LicenseType`) or snake only for DB.
- Keep data‑service classes slim; business logic lives outside data layer.

## C# & .NET patterns
- Favor expression‑bodied members for trivial getters.
- Dispose resources with **`await using`** where applicable.
- No `async void` except top‑level event handlers.
- Guard clauses first; avoid nested `if` pyramids.
- Use `ConfigureAwait(false)` in library code but **not** in ASP.NET endpoints.

## EF Core
- Always chain **`.Include()`** / `.ThenInclude()` for eager loading as soon as you query.
- Use **`AsNoTracking()`** for read‑only queries.
- Catch N + 1 issues: flag any loop that triggers DB calls.
- Prefer server‑side filtering and pagination (`Skip/Take`) before materialising collections.

## Performance & reliability
- Suggest **cancellation tokens** on externally‑reachable async methods.
- Spot redundant LINQ evaluations (`ToList()` then `.Where()`).
- Warn if a potentially hot path lacks caching (check Redis helper usage).
- Flag synchronous I/O inside ASP.NET request pipeline.

## Security & secrets
- No connection strings or keys checked into code.  
  If present, recommend using ParameterResource / Key Vault.
- Validate user‑supplied input before DB calls; highlight risks of string concatenation.

## Testing
- For every bug‑fix PR, request a **regression unit test**.
- New public methods should ship with at least one happy‑path test and one edge‑case test.
- Enforce current coverage floor (≥ 80 % on touched projects).

## Documentation & comments
- XML doc‑comments only on public APIs; omit on privates unless non‑obvious.
- If an algorithm is complex, request a short “why we do it this way” comment above it.

## Pull‑request etiquette
1. Summarise the review at the end:  
   `✅ LGTM` / `❓ Needs minor changes` / `❌ Required changes`.
2. Use GitHub suggestions feature for quick fixes.
3. Never reject solely on stylistic grounds if **dotnet format** would auto‑fix it.
